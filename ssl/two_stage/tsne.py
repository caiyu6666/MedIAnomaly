...
def run_training(data_type="zhanglab",
    ...
    model_name = f"model-{data_type}-{cutpate_type.__name__}-seed_{seed}" + '-{date:%Y-%m-%d_%H_%M_%S}'.format(
    ...
    model_path = glob.glob(os.path.join(out_dir, "*.tch"))[0]  # use one trained model
    # augmentation:
    min_scale = 1
    g = torch.Generator()
    g.manual_seed(seed)
    ...
    if data_type in ['rsna', 'vin', 'brain', 'lag']:
        if data_type == 'rsna':
            ...
            data_path = os.path.join(data_path, "BraTS_AD")
        else:
        ...
        data_path = os.path.join(data_path, "Camelyon16_AD")
    ...
    with torch.no_grad():
        ...
        for data_batch in dataloader:
            data_syn = data_batch[1].to(device)
            embed, _ = model(data_syn)
            # embeds_syn.append(embed.cpu())
            embeds.append(embed.cpu())
            labels.append(torch.tensor([-1] * embed.shape[0]))

            n += embed.shape[0]
            if n >= 1000:
                break

        # embeds_syn = torch.cat(embeds_syn).numpy()
        ...
        # embeds_normal = test_embeds[labels == 0].numpy()
        # embeds_abnormal = test_embeds[labels == 1].numpy()

    # rept vis
    # size0, size1, size2 = embeds_syn.shape[0], embeds_normal.shape[0], embeds_abnormal.shape[0]
    # repts = np.concatenate([embeds_syn, embeds_normal, embeds_abnormal], axis=0)  # Nxd
    tsne = TSNE(n_components=2).fit_transform(embeds)  # Nx2
    tsne_syn = tsne[np.where(labels == -1)]
    tsne_normal = tsne[np.where(labels == 0)]
    tsne_abnormal = tsne[np.where(labels == 1)]
    plt.rcParams['font.family'] = 'Times New Roman'
    plt.rcParams.update({'font.size': 14})
    # plt.scatter(tsne_syn[:, 0], tsne_syn[:, 1], color='y', label="CutPaste", s=2)
    syn_name = {"anatpaste": "AnatPaste", "normal": "CutPaste"}[args.variant]
    plt.scatter(tsne_syn[:, 0], tsne_syn[:, 1], color='y', label=syn_name, s=2)
    # plt.scatter(tsne_syn[:, 0], tsne_syn[:, 1], color='y', label="Synthetic", s=2)
    plt.scatter(tsne_normal[:, 0], tsne_normal[:, 1], color='b', label="Normal", s=2)
    plt.scatter(tsne_abnormal[:, 0], tsne_abnormal[:, 1], color='r', label="Abnormal", s=2)
    plt.xticks([])
    plt.yticks([])
    ...
    #         model.eval()
    #         test_roc_auc, test_ap, cls_auc, cls_ap = eval_model(model_name, data_type, device=device,
    #                                                             save_plots=False,
    #                                                             size=size,
    ...
    #
    # flops_encoder, params_encoder = profile(model.resnet18, inputs=(example_in,))
    ...
    #            "cls_ap": cls_ap}
    # result_path = os.path.join(out_dir, model_name+".txt")
...
if __name__ == '__main__':
    ...
    all_types = ['rsna', 'vin', 'brain', 'lag', 'brats']
