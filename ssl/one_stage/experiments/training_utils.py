...
def train_and_save_model(model, optimizer, loss_func, loader_train, fname, out_dir,
    ...
    for epoch in tqdm(range(num_epochs), desc='Epoch'):
        model.train()
        for batch_idx, (data, _, label, _) in enumerate(tqdm(loader_train, desc='Batch') if tqdm_batch else loader_train):
        ...
        if (epoch + 1) % save_freq == 0 or epoch == 0:
            model.eval()
            with torch.no_grad():
                data, _, label, _ = next(iter(loader_train))
                if reshape is not None:
                    data = data.view(-1, *reshape)
                    label = label.view(-1, *reshape)
                data = data.to(device)
            ...
            # if save_intermediate_model and (epoch + 1) > 200:
            #     torch.save({
            #         'epoch': epoch,
            #         'model_state_dict': model.state_dict(),
            #         'optimizer_state_dict': optimizer.state_dict(),
            #         'scheduler_state_dict': scheduler.state_dict() if scheduler is not None else None,
            #     }, os.path.join(out_dir, fname[:-3] + "_{}".format(epoch)+".pt"))
